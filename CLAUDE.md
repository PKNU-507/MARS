...
과목별로 **별도 세션**이 자료를 만들고, **이 세션이 배포**한다. 접점은 각 과목 폴더의
**`홈페이지_배포이력.md`** 하나다.
