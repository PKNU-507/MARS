...
- **과목 세션**: **이미 배포된** 자료를 고칠 때만 `_v2`, `_v3` … 를 붙인다(이전 판 보존).
  ...
  ※ 선형시스템 **강의노트 책**은 예외 — 책 세션(`_MARS\_Book\CLAUDE.md`)이 직접 배포한다.
...
> `--dry-run` 미리보기, 커밋 전 `href="files/…"` 링크 무결성 검사, `--title` ↔ `material-name` 일치 검사, 중단 뒤 `--resume`,
> 메모 content_id 가 낡았을 때 `--trust-file`). 검증은 전부 쓰기 전에 끝나고, 이력 기록은 push 앞에 한다.
> 사용법은 `홈페이지_배포이력.md` 끝의 「갱신할 때 같이 고칠 곳」.
